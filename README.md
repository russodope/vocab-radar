...
| **拖选** 多词短语（如 `no-brainer`）| 同上（短语翻译可在 popup 关掉） |
| 弹窗"我认识了" | 词标记为 familiar，不再高亮 |
| 弹窗"完全掌握" | 词标记为 graduated，永久不再高亮 |
| 弹窗"不用记" | 从词库**彻底删除**，不计入统计（误操作时用） |
| 鼠标悬停高亮词 | 300ms 后弹出简短释义 tooltip（用之前缓存的释义，不调 API）|
| 点击 popup 图标 | 看词汇统计 + 调 BYOK key / 语言对 / 行为 / 备份 |
...
如果你已经把某个词标为 `familiar` 或 `graduated`，**下次再划这个词** = 强信号说明你其实没真掌握，状态会**自动回到 `learning`** 并恢复高亮。

### 备份 / 跨设备迁移

popup 的"备份与迁移"区有两个按钮：

- **导出词库** —— 下载 `vocab-radar-YYYY-MM-DD.json`，包含所有词 + 查询历史
- **导入词库** —— 选这个 JSON 文件，词库合并进当前设备

换电脑就：旧设备导出 → 传文件 → 新设备装扩展 → 粘 key → 导入。整个流程几分钟。

> API key 和语言偏好不会被导出（它们是设备级偏好）—— 新设备粘一次 key 即可。
...
- [x] **v0.2.x** — UX 打磨：短语开关 / 不用记按钮 / 缓存命中 / max_tokens 上限 / 缓存语言敏感 / 扩展更新自动重注入 / 状态机自动降级 / 备份导入导出
- [ ] **v0.3** — 自动时间降级（familiar 30 天没碰自动退回 learning）
